...
public class HTMLReporter extends AbstractReporter {
    ...
    private void createResults(List<ISuite> suites,
        ...
        for (ISuite suite : suites) {
            int groupIndex = 1;
            for (String group : new ReportNGUtils().getGroupNames(suite)) {
                int resultIndex = 1;
                for (ISuiteResult result : new ReportNGUtils().getResultsByGroup(suite).get(group)) {
                    ...
                    if (!onlyShowFailures || failuresExist) {
                        ...
                        String fileName = String.format("suite%d_group%d_test%d_%s", suiteIndex, groupIndex, resultIndex, RESULTS_FILE);
